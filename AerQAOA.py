...
# ============================================================
# 3) Noise model and noisy simulator
# ============================================================
...
# IMPORTANT: Do not fix seed_simulator here; pass it per .run() for run-level reproducibility.
NOISY_SIM = AerSimulator(noise_model=noise_model)

def transpile_for_noisy_qaoa(qc, seed_transpiler=999):
    return transpile(
        qc,
        seed_transpiler=seed_transpiler,
        optimization_level=1,
        basis_gates=["h", "rz", "rx", "rzz"]
    )
...
# ============================================================
# 5) Noisy shot-based objective
# ============================================================

def qaoa_objective_noisy(theta, p, shots_obj, seed_sim, n_batches, seed_transpiler=999):
    gammas = theta[:p]
    betas = theta[p:]
    ...
    for b in range(n_batches):
        ...
        tqc = transpile_for_noisy_qaoa(qc, seed_transpiler=seed_transpiler)
        ...
        mean_e = expected_energy_from_counts_xorder(counts_xorder)
        vals.append(mean_e / ENERGY_SCALE)

    return float(np.mean(vals))

# ============================================================
# 6) Stats helpers (4 metrics over 30 runs)
# ============================================================
...
# ============================================================
# 8) Run 30 experiments (noisy)
# ============================================================

# Keep these consistent with the noisy baseline for fair comparison.
p = 2
num_restarts = 8
shots_obj = 2048
shots_final = 8192
...
N_RUNS = 30
BASE_SEED = 12345  # use same BASE_SEED across methods for paired comparison
...
for run_id in range(N_RUNS):
    ...
    # Final sampling seed: one per run.
    ...
    tfinal_qc = transpile_for_noisy_qaoa(final_qc, seed_transpiler=seed_transpiler)

    final_result = NOISY_SIM.run(
        tfinal_qc,
        shots=shots_final,
...
print("\n" + "="*70)
print(f"[Summary over {N_RUNS} runs | Noisy Aer | shots_final={shots_final}]")
print("="*70)
